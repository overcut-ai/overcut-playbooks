...
❌ **DO NOT** use `list_dir` or `read_file` on directories — chunk file list comes from the previous step's output
❌ **DO NOT** use `read_file` on source code files — only chunk files
